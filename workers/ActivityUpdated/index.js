...
const states = { };
logger.info('registering consumer for activity updated');
co(function* initWrapped() {
  ...
  // assert the queue
  const qok = yield channel.assertQueue(config.ROUTING_KEYS.ACTIVITY_UPDATED.queueName);
  logger.debug('Assert Queue response', helper.stringify(qok));
  // bind the queue to exchange
  yield channel.bindQueue(config.ROUTING_KEYS.ACTIVITY_UPDATED.queueName,
    config.DEFAULT_EXCHANGE, config.ROUTING_KEYS.ACTIVITY_UPDATED.key);
  return channel;
}).then((channel) => {
  states.connection = CONNECTION_STATE.ACTIVE;
  states.channel = CHANNEL_STATE.ACTIVE;
  // wait for any messages on the queue
  channel.consume(config.ROUTING_KEYS.ACTIVITY_UPDATED.queueName, (msg) => {
